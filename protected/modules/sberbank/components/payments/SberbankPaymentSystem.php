...
class SberbankPaymentSystem extends PaymentSystem
{
    ...
    public function processCheckoutPay(Payment $payment, CHttpRequest $request)
    {
        sleep(5);
        $orderId = $request->getParam('orderId');
        if ($orderId===null) {
        ...
        $sbank = new Sberbank($payment);
        $order = OrderPay::model()->findByAttributes(['payId'=>$orderId]);
        ...
        if ($sbank->getPaymentStatus($request)) {
            $order->status = 1;
            $order->save();
            Yii::app()->mailMessage->raiseMailEvent('oplatit-onlayn', [
                'number' => $order->number,
                'name'   => $order->name,
                'email'  => $order->email,
                'phone'  => $order->phone,
