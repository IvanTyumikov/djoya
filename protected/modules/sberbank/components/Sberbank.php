...
class Sberbank
{
    ...
    public function getUrlPay(OrderPay $order)
    {
        $data = [
            'userName' => $this->userName,
            'password' => $this->password,
            'returnUrl' => Yii::app()->createAbsoluteUrl('/order/orderPay/process'),
            'failUrl' => Yii::app()->createAbsoluteUrl('/order/orderPay/process'),
            'sessionTimeoutSecs' => $this->sessionTimeoutSecs,
            'language' => $this->language,
            'orderNumber' => $order->number,
            'amount' => $order->cost * 100,
            'jsonParams' => json_encode(['email' => $order->email]),
            'description' =>  Yii::t('default', 'Оплата заказа #{n}, пользователь {name} {phone} {email}', [
                '{n}'     => $order->number,
                '{name}'  => $order->name,
                '{phone}' => $order->phone,
                '{email}' => $order->email,
        ...
        if (!isset($sessionData['formUrl'])) {
            Yii::log(Yii::t('default', 'Session ID is not defined.'), CLogger::LEVEL_ERROR);
            return false;
        }

        if (isset($sessionData['orderId'])) {
            $order->payId = $sessionData['orderId'];
            $order->save(false);
        }
