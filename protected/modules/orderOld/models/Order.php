 ...
 * @property integer $number_track
 * @property string $address_obj
 * @property string $sub_delivery_data
 * @property string $delivery_pvz_data
 * @property string $track
 * @property text $delivery_data
 * @property integer $send_1c
 *
...
class Order extends yupe\models\YModel
{
    ...
    const USER_STATUS_REGISTER = 0; // Пользователь не зарегистрирован
    const USER_STATUS_LOGIN = 1; // Пользователь зарегистрирован, залогиниться
    ...
    {
        return [
            ['name, email, phone, zipcode, country, region, city, street, house, apartment, fullAddress', 'filter', 'filter' => 'trim'],
            ['name, email, phone, zipcode, country, region, city, street, house, apartment, comment', 'filter', 'filter' => [$obj = new YPurifier(), 'purify']],
            ['status_id, delivery_id', 'required'],
            ['name, email, phone', 'required', 'on' => self::SCENARIO_USER],
            ['email', 'email'],
            [
                'delivery_id, separate_delivery, payment_method_id, paid, user_id, couponId, manager_id, userStatus, send_1c',
                'numerical',
                'integerOnly' => true,
            ],
            ['delivery_price, total_price, discount, coupon_discount', 'store\components\validators\NumberValidator'],
            ['name, phone, email, region, city, street', 'length', 'max' => 255],
            ...
            ['url', 'unique'],
            ['address_obj, sub_delivery_id, sub_delivery_data, delivery_pvz_data, number_track, track, delivery_data', 'safe'],
            [
            ...
            ['phone', 'match', 'pattern' => '/\+7\(\d{3}\)\d{3}\-\d{2}\-\d{2}$/', 'message' => 'Не правильный формат телефона'],
//            ['phone', 'checkPhone'],
            [
    ...
    public function attributeLabels()
    {
        return [
            ...
            'name' => Yii::t('OrderModule.order', 'ФИО'),
            ...
            'region' => Yii::t('OrderModule.order', 'Область'),
            'city' => Yii::t('OrderModule.order', 'Населенный пункт'),
            ...
            'fullAddress' => Yii::t('OrderModule.order', 'Город получения'),
            'weight' => Yii::t('OrderModule.order', 'Общий вес товаров (г.)'),
        ];
    }
    ...
    public function beforeSave()
    {
        ...
        if ($this->getIsNewRecord()) {
            ...
            if ($this->getScenario() === self::SCENARIO_USER) {
                ...
                $this->separate_delivery = null;
//                if ($this->delivery) {
//                    $data = $this->delivery->createOrder($this);
//                    $this->delivery_data = serialize($data);
//
//                    $this->delivery_price = $this->delivery->getCost($this);
...
//                }
    ...
    {
        $fullAddress = CJSON::decode($this->address_obj);

        if(!$this->zipcode) {
            $this->zipcode = $fullAddress['postal_code'] ?? null;
        }
        if(!$this->country) {
            $this->country = $fullAddress['country'] ?? null;
        }
        if(!$this->region) {
            $this->region = $fullAddress['region_with_type'] ?? null;
        }
        if(!$this->city) {
            $this->city = $fullAddress['city_with_type'] ?? $fullAddress['settlement_with_type'];
        }
        if(!$this->street) {
            $this->street = $fullAddress['street_with_type'] ?? null;
        }
        if(!$this->house) {
            $this->house = $fullAddress['house'] ?? null;
        }
        if(!$this->apartment) {
            $this->apartment = $fullAddress['flat'] ?? null;
        }
    }
    ...
    {
        return [
            self::USER_STATUS_REGISTER => 'Я новый покупатель',
            self::USER_STATUS_LOGIN => 'Я уже зарегистрирован',
        ];
    }

    public function validateDelivery()
    {
        foreach ($_POST as $modelName => $value) {
            if (preg_match('/Form$/', $modelName)) {
                $model = new $modelName;
    ...
    {
        $phone = Yii::app()->dadata->cleatPhone($this->$attribute);
        $types = [
            'Мобильный',
            'Стационарный',
            'Прямой мобильный',
        ...
        if (array_search($type, $types) === false) {
            $this->addError('phone', 'Неверный телефон');
        }
    }

    public function getWeightPosition()
    {
        $weight = 0;
        foreach (Yii::app()->cart->getPositions() as $position) {
            if ($position->weight) {
                $weight += $position->weight * $position->getQuantity();
            } else {
                $weight += 100 * $position->getQuantity();
            }
        }
    ...
    public function getDeliveryData()
    {
        if ($this->delivery_data) {
            return unserialize($this->delivery_data);
        }
    }
}
