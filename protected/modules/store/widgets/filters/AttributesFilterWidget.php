...
class AttributesFilterWidget extends \yupe\widgets\YWidget
{
    ...
    public function run()
    {
        ...
        if ('*' === $this->attributes) {
            $this->attributes = AttributeStore::model()->with(['options.parent', 'group'])->findAll([
        ...
        foreach ($this->attributes as $attribute) {
            ...
            $model = is_string($attribute) ? AttributeStore::model()->findByAttributes([
                'name' => $attribute,
            ...
            if ($model) {
                switch ($model->type) {
                    ...
                    case AttributeStore::TYPE_CHECKBOX_LIST:
                        $this->widget(
                            'application.modules.store.widgets.filters.CheckboxListFilterWidget',
                            ['attribute' => $model]
                        );
                        break;
