...
class AttributeFilter extends CApplicationComponent
{
    ...
    const MAIN_SEARCH_PARAM_NEW        = 'news_product';
    const MAIN_SEARCH_PARAM_RECOMENDED = 'hits_product';
    const MAIN_SEARCH_PARAM_DISCOUNT   = 'discounts_product';
    ...
    public function getMainSearchParams()
    {
        return [
            ...
            self::MAIN_SEARCH_PARAM_DISCOUNT   => 'is_discount',
            self::MAIN_SEARCH_PARAM_NEW        => 'is_new',
            self::MAIN_SEARCH_PARAM_RECOMENDED => 'is_hit',
        ];
    }

    /**
     * @param AttributeStore $attribute
     * @param null $mode
     * @return null|string
     */
    public function getFieldName(AttributeStore $attribute, $mode = null)
    {
        $type = (int)$attribute->type;

        if ($type === AttributeStore::TYPE_SHORT_TEXT) {
            return sprintf($this->stringTemplate, $attribute->name);
        }

        if ($type === AttributeStore::TYPE_CHECKBOX) {
            return sprintf($this->checkboxTemplate, $attribute->name);
        }

        if ($type === AttributeStore::TYPE_NUMBER) {
     ...
     * @return mixed
     */
    public function getFieldValue(AttributeStore $attribute, $mode = null)
    {
        if (null !== $mode) {
     ...
     * @return bool
     */
    public function isFieldChecked(AttributeStore $attribute, $value = null)
    {
    ...
    public function getTypeAttributesForSearchFromQuery(CHttpRequest $request)
    {
        ...
        if (false === $attributes) {
            ...
            $models = AttributeStore::model()->findAll(
                ['select' => ['name', 'id', 'type']]
            );
