...
class DCategoryTreeBehavior extends DCategoryBehavior
{
    ...
    public function getMenuList($sub = 0, $parent = 0)
    {
        ...
        // Исключаем из выборки mostInteresting - id категорий из настроек модуля
        $ids = explode(',', Yii::app()->getModule('store')->mostInteresting);
     ...
     * @return array
     */
    public function getMyMenuList($sub = 0, $parent = 0)
    {
        $criteria = $this->getOwnerCriteria();
        ...
        return $this->_getMyMenuListRecursive($categories, $parent, $sub);
    }
     ...
     * @return array
     */
    protected function _getMyMenuListRecursive($items, $parent, $sub)
    {
        $parent = (int)$parent;
        $resultArray = [];
        if (isset($items[$parent]) && $items[$parent]) {
            $c = count($items[$parent]);
            foreach ($items[$parent] as $item) {
     ...
     * @return array
     */
    protected function _getMenuListRecursive($items, $parent, $sub, $isParent = true)
    {
        $parent = (int)$parent;
        $resultArray = [];
        if (isset($items[$parent]) && $items[$parent]) {
            ...
            foreach ($items[$parent] as $item) {
                ...
                $resultArray[$item->getPrimaryKey()] = [
                        'id' => $item->getPrimaryKey(),
                        'label' => $isParent ? '<span>' . $item->name . '</span>
                            <svg class="arrow_menu">
                        ...
                        'itemOptions' => ['class' => 'item_'.$item->getPrimaryKey()],
                        'linkOptions' => $active ? ['rel' => 'nofollow'] : [],
                        'template' => $isParent ? " <img src='{$item->getImageUrl()}' alt=''>{menu}" : '{menu}'
