 ...
 * @property integer $gallery_category
 *
 * @method getImageUrl
 *
...
class Product extends yupe\models\YModel implements ICommentable
{
    ...
    const IS_DELIVERY_YES = 1;
    const IS_DELIVERY_NO = 0;
    ...
    const STATUS_NOT_IN_NEW = 0;
    const STATUS_IN_NEW = 1;

    const STATUS_NOT_IN_HIT = 0;
    const STATUS_IN_HIT = 1;

    const STATUS_NOT_IN_DISCOUNT = 0;
    const STATUS_IN_DISCOUNT = 1;
    ...
    public function rules()
    {
        return [
            ...
            [
                'name, title, description, short_description, slug, price, discount_price, discount, data, status, is_special, is_new, is_hit',
                'filter',
                'filter' => 'trim',
            ],
            [
                'name, title, short_description, slug, price, discount_price, discount, data, status, is_special, is_new, is_hit',
                'filter',
                'filter' => [$obj = new YPurifier(), 'purify'],
            ],
            ['name, slug', 'required'],
            ['price_result, video', 'safe'],
            [
                'status, is_special, producer_id, type_id, in_stock, category_id, is_new',
                'numerical',
                'integerOnly' => true,
            ],
            ...
            [
                'name, title, meta_keywords, meta_title, meta_canonical, image, image_alt, image_title, video',
                'length',
                'max' => 250,
            ],
            [
            ...
            ['meta_canonical', 'url'],
            ['raiting, visits, gallery_category', 'safe'],
            [
                'id, type_id, producer_id, sku, name, slug, price, discount_price, discount, short_description, description, data, is_special, length, height, width, weight, quantity, in_stock, status, create_time, update_time, meta_title, meta_description, meta_keywords, category_id, is_new, is_hit',
                'safe',
                'on' => 'search',
            ],
        ];
    ...
    public function relations()
    {
        return [
            ...
            'tabs' => [self::HAS_MANY, 'ProductTabs', 'product_id'],
            'variants' => [
                self::HAS_MANY,
                'ProductVariant',
                ['product_id'],
            ...
            'photosReviews' => [self::HAS_MANY, 'ProductPhotosReviews', 'product_id'],
            'photosReviewsMarketplace' => [self::HAS_MANY, 'ProductPhotosReviewsMarketplace', 'product_id']
    ...
    public function attributeLabels()
    {
        return [
            ...
            'raiting' => Yii::t('StoreModule.store', 'Rating'),
            'visits' => Yii::t('StoreModule.store', 'Visits'),
            'is_new' => Yii::t('StoreModule.store', 'Новинка'),
            'is_hit' => Yii::t('StoreModule.store', 'Хит продаж'),
            'video' => Yii::t('StoreModule.store', 'Ссылка на видео'),
            'gallery_category' => Yii::t('StoreModule.store', 'Категория галереи'),
            'is_delivery' => Yii::t('StoreModule.store', 'Is delivery'),
        ];
    }
    ...
    public function search()
    {
        ...
        $criteria->compare('t.raiting', $this->raiting);
        $criteria->compare('t.visits', $this->visits);
        $criteria->compare('t.price_result', $this->price_result);
        $criteria->compare('t.is_new', $this->is_new);
        $criteria->compare('t.is_hit', $this->is_hit);
    ...
    public function beforeValidate()
    {
        foreach ($this->getTypeAttributes() as $attribute) {
            ...
            if ($attribute->isType(AttributeStore::TYPE_CHECKBOX)) {
                continue;
            }
    ...
    public function attribute($attribute, $default = null)
    {
        ...
        $attributeName = $attribute instanceof AttributeStore ? $attribute->name : $attribute;
    ...
    public function beforeDelete()
    {
        ...
        foreach ($this->photosReviews as $image) {
            $image->delete();
        }
    ...
    {
        $city = Yii::app()->getComponent('urlManager')->getCityFromUrl();
        if (!empty($city) && $this->meta_canonical && stristr($this->meta_canonical, 'https://test.ru')) {
            return 'https://test.ru/' . $city . explode('https://test.ru', $this->meta_canonical)[1];
        }
    ...
    public function getVariantsGroup()
    {
        ...
        foreach ((array)$this->variants as $variant) {
            if ((int)$variant->type !== ProductVariant::TYPE_IMAGES_RELATION) {
     ...
     * @return array
     */
    public function getVariantsColorGroup()
    {
        $variantsGroups = [];
    ...
    public function getVariantsOptions()
    {
        ...
        foreach ((array)$this->variants as $variant) {
            $options[$variant->id] = [
                ...
                'data-name' => AttributeOption::model()->findByAttributes(['id' => $variant->attribute_value])->value,
                'data-id' => $variant->type == ProductVariant::TYPE_IMAGES_RELATION ? $variant->attribute_value : $variant->option_id,
                'option-id' => AttributeOption::model()->findByAttributes(['id' => $variant->attribute_value])->id,
    ...
    /**
     * Собираю необходимый массив для select списка опций цвета (Тип 2 - меняет базовую цену)
     * @return array
     */
    public function getColorOptionsList()
    {
        $variantsTypeColorArray = [];

        foreach ($this->variantsOptions as $count => $value) {
            //Если модификатор вариантов "Связать с изображением"
            if ($value['data-type'] == ProductVariant::TYPE_IMAGES_RELATION) {
                $variantsTypeColorArray += [
                    $value['variant-id'] => $value['data-id'],
                ];
            }
        }

        return $variantsTypeColorArray;
    }

    /**
     * Возвращаю массив с вариантами, у которых модификатор - связать с изображениями
     * @return array
     */
    public function getOptionsListForImagesRelation($data)
    {
        $listArray = [];
        foreach ($data as $key => $value) {
            if ($value->type == ProductVariant::TYPE_IMAGES_RELATION) {
                $option = AttributeOption::model()
                    ->findByAttributes([
    ...
    {
        $this->price_result = $this->getResultPrice();
     ...
     * @return array
     */
    public function getInNewsList()
    {
        return [
            self::STATUS_IN_NEW => Yii::t('StoreModule.store', 'Новинка'),
        ];
    }
    /**
     * @return array
     */
    public function getInHitsList()
    {
        return [
            self::STATUS_IN_HIT => Yii::t('StoreModule.store', 'Хит продаж'),
        ];
    }
    /**
     * @return array
     */
    public function getInDiscountsList()
    {
        return [
            self::STATUS_IN_DISCOUNT => Yii::t('StoreModule.store', 'Со скидкой'),
        ];
    }

    public function getBenefit()
    {
        $resultPrice = $this->getBenefitPrice();
        $discount = ($this->discount) ? "(" . str_replace('.00', '', number_format($this->discount, 2, '.', ' ')) . "%)" : "";

        return "Выгода {$resultPrice} руб {$discount}";
    }

    public function getBenefitPrice()
    {
        $resultPrice = $this->getResultPrice();
        $basePrice = $this->getBasePrice();

        $resultPrice = $basePrice - $resultPrice;
    ...
    {
        return [
            self::IS_DELIVERY_NO => ' Нет',
            self::IS_DELIVERY_YES => 'Да',
        ];
    }
    ...
    public function afterSave()
    {
        ...
        $this->imagesCreate();
        $this->imagesCreateMarketplace();

        return parent::afterSave();
    }

    private function imagesCreate()
    {
        $imageFiles = CUploadedFile::getInstancesByName('ProductPhotosReviews');
        ...
        foreach ($imageFiles as $img) {
            ...
            $name = sprintf('%s.%s', uniqid(), $img->getExtensionName());

            $path = Yii::getPathOfAlias('webroot') . '/uploads/image/imagesPhotosReviews/' . $name;

            $pathImg =  '/uploads/image/imagesPhotosReviews/' . $name;

            $img->saveAs($path);

            $model = new ProductPhotosReviews();
        ...
        $data = Yii::app()->getRequest()->getPost('ProductPhotosReviews');
        foreach ($data as $id => $attributes) {
            $model = ProductPhotosReviews::model()->findByPk($id);
            if ($model === null) {
                continue;
            }
    ...
    private function imagesCreateMarketplace()
    {
        $imageFiles = CUploadedFile::getInstancesByName('ProductPhotosReviewsMarketplace');
        ...
        foreach ($imageFiles as $img) {
            ...
            $path = Yii::getPathOfAlias('webroot') . '/uploads/image/imagesPhotosReviewsMarketplace/' . $name;

            $pathImg =  '/uploads/image/imagesPhotosReviewsMarketplace/' . $name;

            $img->saveAs($path);

            $model = new ProductPhotosReviewsMarketplace();
        ...
        $data = Yii::app()->getRequest()->getPost('ProductPhotosReviewsMarketplace');
        foreach ($data as $id => $attributes) {
            $model = ProductPhotosReviewsMarketplace::model()->findByPk($id);
            if ($model === null) {
                continue;
            }
            $model->setAttributes($attributes);
