...
class AttributeStore extends \yupe\models\YModel
{
    /**
     *
     */
    ...
    */
    ...
    public function getTypesList()
    {
        return [
            ...
            self::TYPE_COLOR_LIST => Yii::t('StoreModule.store', 'Список цветов (множественный)'),
        ];
    }
    ...
    {
        return $this->type == self::TYPE_DROPDOWN || $this->type == self::TYPE_CHECKBOX_LIST || $this->type == self::TYPE_COLOR_LIST;
    }
    ...
    public function getOptionsList($category)
    {
     if ($category===null) {
                return $this->options;
            }
            ...
            $ids = array_column(Yii::app()
                ...
                ->select('id')
                ->from('{{store_product}}')
                ->where(['in', 'category_id', !empty($category->getChildsArray()) ? $category->getChildsArray() : $category->id])
                ->queryAll(), 'id');
    ...
    public function isTypeList()
    {
        ...
        if($this->type == self::TYPE_COLOR_LIST){
            $result = true;
        }
