...
class ProductBackendController extends yupe\components\controllers\BackController
{
    ...
    public function accessRules()
    {
        return [
            ...
            [
                'allow',
                'actions' => ['update', 'inline', 'sortable', 'deleteImage', 'batch','deleteHaracteristic', 'deleteTabs'],
    ...
    public function actionCreate()
    {
        ...
        if (Yii::app()->getRequest()->getIsPostRequest() && Yii::app()->getRequest()->getPost('Product')) {
            ...
            if ($model->saveData($attributes, $typeAttributes, $variants, $categories)) {
                ...
                $this->updateProductHaracteristics($model);
                $this->updateProductTabs($model);
    ...
    public function actionUpdate($id)
    {
        ...
        if (Yii::app()->getRequest()->getIsPostRequest() && Yii::app()->getRequest()->getPost('Product')) {
            ...
            $attributes = Yii::app()->getRequest()->getPost('Product');
            $typeAttributes = Yii::app()->getRequest()->getPost('AttributeStore', []);
            $variants = Yii::app()->getRequest()->getPost('ProductVariant', []);
            $categories = Yii::app()->getRequest()->getPost('categories', []);
    ...
    /**
     * @param Haracteristics $product
     */
    protected function updateProductHaracteristics(Product $product)
    {
        if (Yii::app()->getRequest()->getPost('ProductHaracteristics')) {



            foreach (Yii::app()->getRequest()->getPost('ProductHaracteristics') as $key => $val) {
                $productHaracteristics = ProductHaracteristics::model()->findByPk($key);
                if (null === $productHaracteristics) {
                    $productHaracteristics = new ProductHaracteristics();
                    $productHaracteristics->product_id = $product->id;
                }
                $productHaracteristics->setAttributes($_POST['ProductHaracteristics'][$key]);


                if (false === $productHaracteristics->save()) {
                    Yii::app()->getUser()->setFlash(\yupe\widgets\YFlashMessages::ERROR_MESSAGE,
                        Yii::t('StoreModule.store', 'Error uploading some haracteristics...'));
                }
            }
        }
    }

    // delete Haracteristics
    public function actionDeleteHaracteristics()
    {
        if (Yii::app()->getRequest()->getIsPostRequest() && Yii::app()->getRequest()->getIsAjaxRequest()) {

            $id = (int)Yii::app()->getRequest()->getPost('id');

            $model = ProductHaracteristics::model()->findByPk($id);
     ...
     * @param Tabs $product
     */
    protected function updateProductTabs(Product $product)
    {
        if (Yii::app()->getRequest()->getPost('ProductTabs')) {



            foreach (Yii::app()->getRequest()->getPost('ProductTabs') as $key => $val) {
                $productTabs = ProductTabs::model()->findByPk($key);
                if (null === $productTabs) {
                    $productTabs = new ProductTabs();
                    $productTabs->product_id = $product->id;
                }
                $productTabs->setAttributes($_POST['ProductTabs'][$key]);


                if (false === $productTabs->save()) {
                    Yii::app()->getUser()->setFlash(\yupe\widgets\YFlashMessages::ERROR_MESSAGE,
                        Yii::t('StoreModule.store', 'Error uploading some tabs...'));
                }
            }
        }
    }
    ...
    // delete Tabs
    public function actionDeleteTabs()
    {
        if (Yii::app()->getRequest()->getIsPostRequest() && Yii::app()->getRequest()->getIsAjaxRequest()) {

            $id = (int)Yii::app()->getRequest()->getPost('id');

            $model = ProductTabs::model()->findByPk($id);
    ...
    public function actionTypeAttributes($id)
    {
        ...
        $noSupported = [AttributeStore::TYPE_FILE, AttributeStore::TYPE_TEXT, AttributeStore::TYPE_CHECKBOX_LIST];
        ...
        foreach ($type->typeAttributes as $attr) {
            ...
            if ($attr->type == AttributeStore::TYPE_DROPDOWN) {
                $types[] = array_merge($attr->attributes, ['options' => $attr->options]);
            } else {
                if (in_array($attr->type, [AttributeStore::TYPE_CHECKBOX, AttributeStore::TYPE_SHORT_TEXT])) {
    ...
    {
        echo `php ../protected/yiic update`;
        Yii::app()->user->setFlash('success', 'Цены успешно обновлены');
        $this->redirect(['index']);
    }

    public function actionDeleteImagesReviews()
    {
        if (Yii::app()->getRequest()->getIsPostRequest() && Yii::app()->getRequest()->getIsAjaxRequest()) {
    ...
    public function actionDeleteImagesReviewsMarketplace()
    {
        if (Yii::app()->getRequest()->getIsPostRequest() && Yii::app()->getRequest()->getIsAjaxRequest()) {
