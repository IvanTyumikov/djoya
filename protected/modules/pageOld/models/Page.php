...
class Page extends yupe\models\YModel
{
    ...
    public function rules()
    {
        return [
            ...
            ['title, meta_title, meta_keywords, layout, view, meta_robots', 'length', 'max' => 250],
    ...
    public function attributeLabels()
    {
        return [
            ...
            'text_mirror' => Yii::t('PageModule.page', 'Текст - mirror editor'),
            'image' => 'Изображение',
     ...
     * @return array
     */
    public function getAllPagesListBySlug($slug = false, $with_links = false)
    {
    ...
    {
        $links = [];
        $links['pages'] = new CActiveDataProvider('Page', [
            'criteria' => [
                'condition' => 'status = 1 and slug != :slug',
                'params' => [':slug' => $this->slug]
            ],
        ]);
        $links['categories'] = new CActiveDataProvider('StoreCategory', [
            'criteria' => [
                'condition' => 'status=1',
            ],
        ]);
        $links['products'] = new CActiveDataProvider('Product', [
            'criteria' => [
                'condition' => 'status=1',
            ],
        ]);
        $links['posts'] = new CActiveDataProvider('Post', [
            'criteria' => [
                'condition' => 'status=1',
            ],
        ]);
