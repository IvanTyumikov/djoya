...
class Order extends yupe\models\YModel
{
    ...
    {
        return [
            ['name, email, phone, country, comment', 'filter', 'filter' => 'trim'],
            // ['name, email, phone, country, comment', 'filter', 'filter' => [$obj = new YPurifier(), 'purify']],
            ...
            // ['comment, notem gift_message', 'length', 'max' => 1024],
    ...
    public function relations()
    {
        return [
            ...
            'productsCount' => [self::STAT, 'OrderProduct', 'order_id'],
    ...
    public function attributeLabels()
    {
        return [
            ...
            'comment' => Yii::t('OrderModule.order', 'Комментарий к доставке'),
            'gift_message' => Yii::t('OrderModule.order', 'Комментарий к подарку'),
            ...
            'country' => Yii::t('OrderModule.order', 'Адрес'),
            'city' => Yii::t('OrderModule.order', 'Город доставки'),
    ...
    public function getCouponDiscount(array $coupons)
    {
        ...
        if ($this->isCouponsAvailable()) {
            foreach ($coupons as $coupon) {
                if ($coupon->type == CouponType::TYPE_SUM) {
                    $sum_coupons = $coupon->value;
                    return $sum_coupons;
                } elseif ($coupon->type == CouponType::TYPE_PERCENT) {
                    foreach ($this->products as $key => $product_order) {

                        $priceProd = round($product_order->getTotalPrice(), 2);

                        $delta = ($coupon->value / 100) * $priceProd;

                        $category_id = Product::model()->findByPk($product_order->product_id)->category_id;

                        $cat_ids = explode(",", Yii::app()->getModule('coupon')->categoryId);

                        foreach ($cat_ids as $cat_id) {
                            if ($category_id == $cat_id) {
                                $coupon_prices[$product_order->product_id] = $delta;
                            }
                        }
                    }
                    $sum_coupons = array_sum($coupon_prices);
    ...
    public function getCouponProductDiscount(array $coupons, $orderSum = null)
    {
        $productsTotalPrice = $this->getProductsCost();
        $delta = 0.00;
            ...
            foreach ($coupons as $coupon) {
                if ($coupon->type == CouponType::TYPE_SUM) {
                    foreach ($this->products as $key => $product_order) {
                        ...
                        $couponProc = $coupon->value / ($orderSum / 100);

                        $delta = ($couponProc / 100) * $priceProd;

                        $newPrice = $priceProd - $delta;
                    }
                    ...
                    foreach ($this->products as $key => $product_order) {
                        ...
                        foreach ($cat_ids as $cat_id) {
                            if ($category_id == $cat_id) {
                                $coupon_prices[$product_order->product_id] = $newPrice;
                            }
                        }
                    }
                    return $coupon_prices;
    ...
    {
        // echo ('<pre>');
        // print_r((float)$this->total_price);
        // exit;
