...
$(document).ready(function () {
    ...
    function getCouponDiscount(coupons) {
        let delta = 0;
        var coupon_prices = 0;

        let prices_product = $('.cart-sidebar__info');
        var categoryIdCoupons = categoryIdCoupon.split(',');

        $.each(prices_product, function (index, e) {
            var elem = $(this).find('.position-sum-price');
            let price_product = parseFloat(elem.text());
            let category_id = elem.data('category-id');
            let productId = $(this).find('.position-id').val();
            ...
            let sum_price_coupon = $(this).find('.sum-price_coupon');

            for (var i = 0; i < products_coup.length; i++) {
                let price_coupon =
                    parseFloat(elem.text()) - parseFloat(products_coup[i]);
                // console.log(typeof(products_coup[i]));
                if (products_coup[i] != 0) {
                    sum_price_coupon.text(price_coupon + ' ₽');
                    elem.css({
    ...
    function getCartTotalCost() {
        ...
        let sum_coupons = getCouponDiscount(coupons);
        ...
        //             break;
        //             case 1: // %
        //             delta += (parseInt(el.value) / 100).toFixed(2) * cost.toFixed(2);
        //             break;
        //         }
        //     }
        // });
        ...
        return sum_coupons > cost ? 0 : cost - sum_coupons;
    }
    ...
    $('.coupon .close').click(function (e) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $('.position-sum-price').css({
                    'text-decoration': 'unset',
                    color: '#373E44',
                });
                $('.sum-price_coupon').remove();
                if (data.result) {
                    updateAllCosts();
                }
            },
