<div class="product__item product-item product-item_hit js-product-item">
        ...
        <div>
            <div class="product-item__image product-item-image">
                ...
                <a href="<?= ProductHelper::getUrl($data); ?>" class="item-image-lists">
                    <span class="item-image-list__list js-image-lists" data-src="<?= $img; ?>" data-srcset="<?= $data->getImageUrlWebp(345, 456, true, null, null, null, $img); ?>" data-key="0">
                    </span>
                    <?php if (count($data->getImages()) > 0) : ?>
                        <?php foreach ($data->getImages() as $key => $image) : { ?>
                                <span class="item-image-list__list js-image-lists" data-src="<?= $image->getImageUrl(345, 456); ?>" data-srcset="<?= $image->getImageUrlWebp(345, 456); ?>" data-key="<?= ($key + 1); ?>">
                                </span>
                                <?php if ($key > 5) break; ?>
                        <?php }
                        endforeach; ?>
